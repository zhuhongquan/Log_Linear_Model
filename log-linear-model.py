...
class LogLinearModel(object):
    ...
    def create_feature_template(self, sentence, tag, position):
        ...
        for i in range(1, len(sentence[position]) - 1):
            ...
            template.append('10:' + cur_tag + sentence[position][0] + '*' + sentence[position][i])  # 第一个字符和当前字符添加到模板中
            template.append('11:' + cur_tag + sentence[position][-1] + '*' + sentence[position][i])  # 最后一个字符和当前字符添加到模板中
    ...
    def get_max_tag(self, sentence, position):
        f_list = [self.create_feature_template(sentence, tag, position) for tag in self.tag_list]
        scores = [self.dot(feature) for feature in f_list]  # 将每个tag对应的得分加起来，得到一个得分列表
        # f_list为当前词对应的各个tag的特征，长度为len(tags)，其中的元素feature为当前词对应某个tag时的特征
        max_tag_index = np.argmax(scores)  # 获得最大的分对应的下标
        return self.tag_list[int(max_tag_index)]

    def get_prob(self, f_list):
        scores = np.array([self.dot(feature) for feature in f_list])  # 将每个tag对应的得分加起来，得到一个得分列表
        s = logsumexp(scores)
    ...
    def SGD_train(self, iteration, batch_size, shuffle, regularization, step_opt, eta, C, stop_iteration):
        ...
        for iter in range(iteration):
            ...
            for i in range(len(data)):
                ...
                f_list = [self.create_feature_template(sentence, tag, j) for tag in self.tag_list]
                prob_list = self.get_prob(f_list)
                for f in gold_feature:
                    if f in self.features:
                        self.g[self.features[f]] += 1
                for feature, p in zip(f_list, prob_list):
                    for f in feature:
                        if f in self.features:
